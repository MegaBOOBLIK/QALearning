...
Если для какого-то кейса надо сделать дополнительную таблицу, наполнить её данными, то делайте )
 ...
 3. Вывести все зарплатные позиции, но работник по ним не на онятно кто её получает.)
...
--where employee_id=employees.id and salary_id=salary.id order by employee_salary.id;
...
join employees on e.id = es.employee_id
